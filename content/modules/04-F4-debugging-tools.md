...
- **page-faults**: обращения к ещё не отображённым страницам — minor (demand-paging / COW из page cache) и major (подкачка из swap). Это **не** промахи TLB: те обрабатываются аппаратным page-walk'ом и считаются отдельными счётчиками `dTLB-load-misses`/`iTLB-load-misses`
...
Частая ловушка: `ulimit -c` по умолчанию `0` → core не создаётся вообще. И ещё: `core_pattern` глобальный, а лимит — на процесс; должны совпасть оба условия.
...
MALLOC_PERTURB_=42 ./prog  # забивать свежую память байтом 42, освобождённую — ~42
...
С glibc 2.26 появился **tcache** (per-thread cache мелких чанков) — он же добавил проверку double-free «в tcache» и поле-ключ для её срабатывания. Сообщение `double free detected in tcache 2` означает: блок уже лежит в кэше освобождённых и его освобождают повторно.
...
Даже **без** санитайзеров glibc поймает это сам — благодаря ключу tcache (с 2.26):
...
Каждое предупреждение снабжено **путём** (interprocedural trace), как анализатор пришёл к дефекту, — это читается как мини-доказательство.
...
Подвох по умолчанию: GCC на `-O2` **выкидывает frame pointer** (`-fomit-frame-pointer` включён). Тогда `--call-graph fp` даёт обрубленные стеки. Лечение: либо собрать продакшн с `-fno-omit-frame-pointer` (стоит ~1% производительности, но возвращает читаемые профили и flame graph'ы), либо использовать `dwarf`/`lbr`.
...
- нет frame pointer (`-fomit-frame-pointer` по умолчанию) → раскрутка по FP невозможна; спасает `.eh_frame` (CFI), который GDB использует и так, но в «голых» функциях (asm, PLT) его может не быть;
