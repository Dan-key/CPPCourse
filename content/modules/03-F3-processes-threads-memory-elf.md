...
| Виртуальная память | гл. 49–50 | `man 2 mmap`, `man 2 mlock` | Как адресное пространство映射 на физику |
| ELF | — | `man 5 elf` | Структура исполняемого файла |
| pthreads | гл. 29–33 | `man 7 pthreads`, `man 3 pthread_create` | POSIX threading API |
| Atomics C11 | — | `man 7 memory_model` | Модель памяти и безопасность без мьютексов |
...
- Файловые блокировки (`fcntl` locks — они привязаны к паре PID+fd)
...
**Следствия COW:**
- `fork()` быстрый — O(1) по данным, копируется только таблица страниц
...
void daemonize(void)
{
    ...
    /*
     * Внук: не является лидером сессии → не может получить
     * управляющий терминал через open("/dev/tty", ...).
     * Настоящий daemon.
     */
...
- `posix_spawn()` — современная замена для fork+exec без промежуточного процесса
...
- Маска сигналов (pending signals очищаются)
...
**TLB (Translation Lookaside Buffer)** — кэш аппаратных трансляций virtual→physical. При попадании трансляция происходит за 1 цикл (вместо 4 обращений к памяти). При переключении контекста (смена CR3) TLB сбрасывается (TLB flush) — одна из причин, почему переключения дороги. ASID (Address Space Identifier) позволяет избежать полного flush на ARM и при KPTI на x86.
...
| `PT_GNU_STACK` | Защита: `p_flags` без `PF_X` → NX на стеке |
...
Первый вызов printf():
    ...
    ; _dl_runtime_resolve():
    ;   найти printf в libc по SONAME + индексу
...
По умолчанию все C11 atomic операции используют `memory_order_seq_cst` (sequentially consistent) — самый строгий, самый медленный. На x86-64 разница минимальна (TSO модель), на ARM/POWER — существенна.
...
struct clone_args args = {
    ...
    .exit_signal = SIGCHLD,
    .pidfd       = (uint64_t)(uintptr_t)&pidfd,  /* создать pidfd */
    .flags       |= CLONE_PIDFD,
};
