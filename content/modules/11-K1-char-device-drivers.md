...
noop_llseek`, либо вернуть `-ESPIPE`, если seek недопустим). Так устроены пайпы (C3)
и tty. (Старый `no_llseek` удалён из ядра 6.8+: теперь незаданный `.llseek` и так
запрещает seek — используй `noop_llseek`, когда seek нужно явно сделать no-op.)
Кольцо + wait queue + лок = классический драйвер символьного устройства-потока.
...
- **`.compat_ioctl`** — 32-битный userspace на 64-битном ядре передаёт структуры с
  **другим** layout (размеры/выравнивание `long`/указателей отличаются). Если драйвер
  обслуживает 32-бит процессы на 64-бит ядре и команды несут **структуры** — нужен
  `.compat_ioctl`, который их конвертирует. Если же все команды передают **только
  указатель** (как `arg`), достаточно `.compat_ioctl = compat_ptr_ioctl` — он сам
  приводит 32-битный указатель (`compat_ptr`) и зовёт твой `unlocked_ioctl`.
...
### 17.4 Модуль из нескольких файлов и зависимости
...
18. Как devtmpfs создаёт `/dev/cppchar`? Зачем `class_create`+`device_create`?
